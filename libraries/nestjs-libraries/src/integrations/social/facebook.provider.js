...
export class FacebookProvider extends SocialAbstract {
    ...
    pages(accessToken) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield fetchPaginated(`https://graph.facebook.com/v20.0/me/accounts?fields=id,username,name,picture.type(large)&limit=100&access_token=${accessToken}`);
                ...
                while (bizUrl) {
                    ...
                    if (bizResponse.data) {
                        for (const business of bizResponse.data) {
                            try {
                                yield fetchPaginated(`https://graph.facebook.com/v20.0/${business.id}/owned_pages?fields=id,username,name,picture.type(large)&limit=100&access_token=${accessToken}`);
                            }
                            catch (_b) {
                                // Continue with other businesses
                            }
                            try {
                                yield fetchPaginated(`https://graph.facebook.com/v20.0/${business.id}/client_pages?fields=id,username,name,picture.type(large)&limit=100&access_token=${accessToken}`);
                            }
                            catch (_c) {
                                // Continue with other businesses
                            }
    ...
    fetchPageInformation(accessToken, data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { id, name, access_token, username, picture: { data: { url }, }, } = yield (yield fetch(`https://graph.facebook.com/v20.0/${pageId}?fields=username,access_token,name,picture.type(large)&access_token=${accessToken}`)).json();
            return {
                id,
                name,
                access_token,
