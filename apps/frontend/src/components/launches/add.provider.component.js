...
import { ApiKeyDto } from "../../../../../libraries/nestjs-libraries/src/dtos/integrations/api.key.dto";
import { useRouter } from 'next/navigation';
import { TopTitle } from "./helpers/top.title.component";
...
export const UrlModal = (props) => {
    ...
    return (<div className="rounded-[4px] border border-customColor6 bg-sixth px-[16px] pb-[16px] relative">
      <TopTitle title={`Instance URL`}/>
      <button onClick={close} className="outline-none absolute end-[20px] top-[20px] mantine-UnstyledButton-root mantine-ActionIcon-root hover:bg-tableBorder cursor-pointer mantine-Modal-close mantine-1dcetaa" type="button">
...
export const CustomVariables = (props) => {
    ...
    const schema = useMemo(() => {
        return object(Object.assign({}, variables.reduce((aIcc, item) => {
            const splitter = item.validation.split('/');
            const regex = new RegExp(splitter.slice(1, -1).join('/'), splitter.pop());
            return Object.assign(Object.assign({}, aIcc), { [item.key]: string()
    ...
    const methods = useForm({
        ...
        values: variables.reduce((acc, item) => (Object.assign(Object.assign({}, acc), (item.defaultValue
            ? {
    ...
    const submit = useCallback((data) => __awaiter(void 0, void 0, void 0, function* () {
        const { url } = yield (yield fetch(`/integrations/social/${identifier}${onboarding ? '?onboarding=true' : ''}`)).json();
        modals.closeAll();
        gotoUrl(`/integrations/social/${identifier}?state=${url}&code=${Buffer.from(JSON.stringify(data)).toString('base64')}${onboarding ? '&onboarding=true' : ''}`);
    }), [variables, onboarding]);
    const t = useT();
    return (<div className="rounded-[4px] relative">
      <FormProvider {...methods}>
        <form className="gap-[8px] flex flex-col pt-[10px]" onSubmit={methods.handleSubmit(submit)}>
          {variables.map((variable) => (<div key={variable.key}>
...
const ExtensionNotFound = () => {
    const modals = useModals();
    const t = useT();
    return (<div className="flex flex-col gap-[16px] pt-[8px]">
...
const ChromeExtensionWarning = ({ onConfirm, onCancel }) => {
    const modals = useModals();
    const t = useT();
    return (<div className="flex flex-col gap-[16px] pt-[8px]">
      <p className="text-[14px] text-textColor/80">
      ...
      <div className="flex gap-[10px] mt-[8px]">
        ...
        <Button type="button" className="flex-1 !bg-transparent border border-tableBorder text-textColor" onClick={() => {
            modals.closeCurrent();
            onCancel();
        }}>
          {t('cancel', 'Cancel')}
...
export const AddProviderComponent = (props) => {
    ...
    const fetch = useFetch();
    const modal = useModals();
    const getSocialLink = useCallback((invite, identifier, isExternal, isWeb3, isChromeExtension, customFields) => () => __awaiter(void 0, void 0, void 0, function* () {
        var _a;
        const onboardingParam = onboarding ? 'onboarding=true' : '';
        const openWeb3 = () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            modal.openModal({
                ...
                children: (<Web3Providers onComplete={(code, newState) => {
                        window.location.href = `/integrations/social/${identifier}?code=${code}&state=${newState}${onboarding ? '&onboarding=true' : ''}`;
                    }} nonce={url}/>),
            });
            return;
        });
        const gotoIntegration = (externalUrl) => __awaiter(void 0, void 0, void 0, function* () {
            const params = [
                ...
                .filter(Boolean)
                .join('&');
            const { url, err } = yield (yield fetch(`/integrations/social/${identifier}${params ? `?${params}` : ''}`)).json();
            if (err) {
        ...
        if (isChromeExtension) {
            const confirmed = yield new Promise((resolve) => {
                modal.openModal({
                    ...
                    children: (<ChromeExtensionWarning onConfirm={() => {
                            resolve(true);
                        }} onCancel={() => {
                            resolve(false);
                        }}/>),
            ...
            if (!extensionId || !((_a = chrome === null || chrome === void 0 ? void 0 : chrome.runtime) === null || _a === void 0 ? void 0 : _a.sendMessage)) {
                modal.openModal({
            ...
            try {
                yield new Promise((resolve, reject) => {
                    chrome.runtime.sendMessage(extensionId, { type: 'PING' }, (response) => {
                        if (chrome.runtime.lastError || !(response === null || response === void 0 ? void 0 : response.status)) {
                ...
                toaster.show(t('extension_not_installed', 'Postiz browser extension is not installed or not reachable.'), 'warning');
                return;
            }
            try {
                const cookieResponse = yield new Promise((resolve, reject) => {
                    chrome.runtime.sendMessage(extensionId, { type: 'GET_COOKIES', provider: identifier }, (response) => {
                ...
                if (!cookieResponse.success) {
                    toaster.show(cookieResponse.error ||
                        t('extension_cookies_missing', 'Could not get cookies. Please log in to the platform first.'), 'warning');
                    return;
                }
                const { url } = yield (yield fetch(`/integrations/social/${identifier}${onboarding ? '?onboarding=true' : ''}`)).json();
                modal.closeAll();
                window.location.href = `/integrations/social/${identifier}?state=${url}&code=${Buffer.from(JSON.stringify(cookieResponse.cookies)).toString('base64')}${onboarding ? '&onboarding=true' : ''}`;
            }
            catch (_c) {
        ...
        if (customFields) {
            modal.openModal({
                ...
                children: (<CustomVariables identifier={identifier} gotoUrl={(url) => router.push(url)} variables={customFields} onboarding={onboarding}/>),
            });
            return;
        }
        yield gotoIntegration();
    }), [onboarding]);
    const t = useT();
    ...
    return (<div className="w-full flex flex-col gap-[20px] rounded-[4px] relative]">
      <div className="flex flex-col">
        <div className="flex justify-center">
          {tiktokProvider ? (<div className="w-full max-w-[360px] rounded-[20px] border border-[#00D4AA]/30 bg-[#161b22] p-[24px] shadow-[0_0_24px_rgba(0,212,170,0.14)]" {...(!!tiktokProvider.toolTip
            ? {
            ...
            : {})}>
              <div className="flex flex-col items-center gap-[16px] text-center">
                <div className="flex h-[88px] w-[88px] items-center justify-center rounded-full bg-black shadow-[0_0_18px_rgba(0,212,170,0.22)]">
                  <img className="h-[52px] w-[52px] rounded-full" src="/icons/platforms/tiktok.png"/>
                </div>
                ...
                <button onClick={getSocialLink(props.invite, tiktokProvider.identifier, tiktokProvider.isExternal, tiktokProvider.isWeb3, tiktokProvider.isChromeExtension, tiktokProvider.customFields)} className="flex h-[52px] w-full items-center justify-center gap-[10px] rounded-[14px] bg-gradient-to-r from-[#00D4AA] to-[#00D4FF] px-[18px] text-[16px] font-[700] text-[#0d1117] transition-transform duration-200 hover:scale-[1.01]">
                  <img className="h-[24px] w-[24px] rounded-full" src="/icons/platforms/tiktok.png"/>
                  <span>{t('connect', 'Connect')} TikTok</span>
                </button>
              </div>
            </div>) : (<div className="w-full rounded-[16px] border border-white/10 bg-[#161b22] p-[20px] text-center text-[14px] text-white/70">
...
//# sourceMappingURL=add.provider.component.js.map