...
import { IntegrationContext } from "./helpers/use.integration";
...
export const ContinueIntegration = (props) => {
    ...
    const [twoStepState, setTwoStepState] = useState(null);
    const [successState, setSuccessState] = useState(null);
    const [isSaving, setIsSaving] = useState(false);
    // Helper to handle navigation - redirects if logged or returnURL exists, otherwise shows inline
    const navigateOrShow = useCallback((path, returnURL, successMessage) => {
        if (returnURL) {
    ...
    const modifiedParams = useMemo(() => {
        ...
        if (provider === 'vk') {
            return Object.assign(Object.assign({}, searchParams), { state: searchParams.state || '', code: searchParams.code + '&&&&' + searchParams.device_id });
        }
        if (provider === 'mewe') {
    ...
    useEffect(() => {
        (() => __awaiter(void 0, void 0, void 0, function* () {
            ...
            let data = yield fetch(`/integrations/social-connect/${provider}`, {
                method: 'POST',
                body: JSON.stringify(Object.assign(Object.assign({}, modifiedParams), { timezone })),
            });
            // If public endpoint fails with specific errors, try authenticated endpoint
            if (data.status === HttpStatusCode.BadRequest) {
                const errorData = yield data.json().catch(() => ({}));
                // "Invalid connection type" means this wasn't started as a public flow
                if (((_a = errorData.message) === null || _a === void 0 ? void 0 : _a.includes('Invalid connection type')) ||
                    ((_b = errorData.message) === null || _b === void 0 ? void 0 : _b.includes('Invalid or expired state'))) {
                    data = yield fetch(`/integrations/social-connect/${provider}`, {
                        method: 'POST',
                        body: JSON.stringify(Object.assign(Object.assign({}, modifiedParams), { timezone })),
                    });
                }
            ...
            const { inBetweenSteps, id, onboarding: resOnboarding, pages, returnURL, extensionToken, } = yield data.json();
            ...
            if (extensionToken &&
                ...
                ((_c = chrome === null || chrome === void 0 ? void 0 : chrome.runtime) === null || _c === void 0 ? void 0 : _c.sendMessage)) {
                try {
    ...
    const onSave = useCallback((data) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            ...
            const response = yield fetch(endpoint, {
                method: 'POST',
                body: JSON.stringify(Object.assign(Object.assign({}, modifiedParams), data)),
            });
            ...
            navigateOrShow(`/launches?added=${provider}&msg=Channel Added${twoStepState.onboarding ? '&onboarding=true' : ''}`, twoStepState.returnURL, 'Channel Added');
        }
        finally {
            setIsSaving(false);
        }
    ...
    const Provider = useMemo(() => {
        return (continueProviderList[provider] ||
            null);
    }, [provider]);
    ...
    if (successState) {
        return (<div className="flex flex-1 items-center justify-center text-white relative overflow-hidden">
        {/* Background gradient decoration */}
        <div className="absolute inset-0 opacity-30">
          <div className="absolute top-[20%] left-[10%] w-[300px] h-[300px] bg-[#E8243C] rounded-full blur-[120px]"/>
          <div className="absolute bottom-[20%] right-[10%] w-[250px] h-[250px] bg-[#00D4FF] rounded-full blur-[120px]"/>
        </div>
        ...
        <div className="relative z-10 text-center">
          ...
          <div className="text-[16px] text-gray-400 max-w-[400px]">
            {successState.message ||
                t('channel_connected_description', `Your ${providerDisplayName} channel has been successfully connected. You can close this window now.`)}
          </div>
        </div>
      </div>);
    }
    ...
    if (twoStepState && Provider) {
        ...
        <div className="relative z-10 w-full max-w-[550px] mx-auto px-[20px]">
          <div className="bg-[#141414] rounded-[16px] p-[32px] flex flex-col gap-[24px]">
            ...
            <IntegrationContext.Provider value={{
              ...
              <Provider onSave={onSave} existingId={[]} initialData={twoStepState.pages} isSaving={isSaving}/>
    ...
    if (error) {
        ...
        <div className="relative z-10 text-center">
          ...
          <div className="text-[16px] text-gray-400 max-w-[400px]">
            {errorMessage ||
                t('you_are_being_redirected_back', 'An error occurred. Please try again.')}
          </div>
    ...
    return (<div className="flex flex-1 items-center justify-center text-white relative overflow-hidden">
      ...
      <div className="relative z-10 text-center">
        ...
        <div className="mt-[32px] flex justify-center">
          <div className="w-[48px] h-[48px] border-[3px] border-[#E8243C] border-t-transparent rounded-full animate-spin"/>
        </div>
      </div>
    </div>);
};
//# sourceMappingURL=continue.integration.js.map