...
export const ContentStudio = () => {
    ...
    const [activeAiTab, setActiveAiTab] = useState('caption');
    ...
    const tiktokAccounts = useMemo(() => {
        if (!integrations)
            return [];
        return integrations.filter((i) => i.providerIdentifier === 'tiktok' || i.identifier === 'tiktok');
    }, [integrations]);
      ...
      <div className="flex-1 flex gap-4 min-h-0">
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          <div className="bg-[#161b22] border border-white/10 rounded-2xl p-5">
            ...
            {/* AI Prompt */}
            <input type="text" value={aiPrompt} onChange={(e) => setAiPrompt(e.target.value)} placeholder="Quel type de contenu l'IA doit-elle générer ou optimiser ?" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-[#00D4AA]/50 mb-4"/>

            {/* AI Tabs */}
            <div className="flex gap-1 mb-4">
              {Object.keys(aiTabLabels).map((tab) => (<button key={tab} onClick={() => setActiveAiTab(tab)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${activeAiTab === tab
              ...
              <div className="flex-1 flex flex-col gap-3">
                ...
                <div className="flex items-center gap-2 bg-white/5 rounded-xl px-3 py-2">
                  <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#E8243C] to-[#00D4AA] flex items-center justify-center text-white text-xs font-bold">
                    {((_b = (_a = tiktokAccounts === null || tiktokAccounts === void 0 ? void 0 : tiktokAccounts[0]) === null || _a === void 0 ? void 0 : _a.name) === null || _b === void 0 ? void 0 : _b[0]) || 'V'}
                  </div>
                  <div className="flex-1">
                    <span className="text-xs text-white/40">Publier sur :</span>
                    <span className="text-xs text-white font-medium ml-1">
                      @{((_c = tiktokAccounts === null || tiktokAccounts === void 0 ? void 0 : tiktokAccounts[0]) === null || _c === void 0 ? void 0 : _c.identifier) || 'VoyageurExcep'}
                    </span>
                  </div>
                ...
                {/* Description area */}
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-[#00D4AA]/50 resize-none" placeholder="Rédigez votre légende ici..."/>
        ...
        <div className="w-[280px] flex flex-col gap-4">
          ...
          <div className="bg-[#161b22] border border-white/10 rounded-2xl p-4">
            ...
            <div className="mb-4">
              ...
              <input type="range" min="0" max="100" value={toneValue} onChange={(e) => setToneValue(Number(e.target.value))} className="w-full h-1 bg-white/10 rounded-full appearance-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-[#00D4AA]"/>
            </div>

            {/* Length slider */}
            <div className="mb-4">
              <p className="text-xs text-white/40 mb-2">Longueur</p>
              <input type="range" min="0" max="100" value={lengthValue} onChange={(e) => setLengthValue(Number(e.target.value))} className="w-full h-1 bg-white/10 rounded-full appearance-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-[#00D4AA]"/>
            </div>
          ...
          <div className="bg-[#161b22] border border-white/10 rounded-2xl p-4">
            ...
            <div className="flex flex-col gap-2">
              {hashtagSuggestions.map((h) => (<div key={h.tag} className="flex items-center justify-between">
          ...
          <div className="bg-[#161b22] border border-white/10 rounded-2xl p-4">
            ...
            <div className="flex flex-col gap-2">
              {scriptSuggestions.map((s, i) => (<div key={i} className="bg-white/5 rounded-lg p-2 text-xs text-white/50 cursor-pointer hover:text-white/70 hover:bg-white/10 transition-colors">
                  {s}
                </div>))}
            </div>
          </div>
...
//# sourceMappingURL=content.studio.js.map