...
import { AttachToFeedbackIcon } from "../new-layout/sentry.feedback.component";
...
const EmbeddedBilling = dynamic(() => import('@gitroom/frontend/components/billing/embedded.billing').then((mod) => mod.EmbeddedBilling), {
    ssr: false,
});
export const FirstBillingComponent = () => {
    ...
    const loadCheckout = useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
        return (yield fetch('/billing/embedded', {
            method: 'POST',
            body: JSON.stringify(Object.assign(Object.assign({ billing: tier, period: period }, (datafast_visitor_id && datafast_session_id
    ...
    const showYouTube = () => {
        modals.openModal({
            ...
            children: (<iframe className="h-full aspect-video min-w-[800px]" src="https://www.youtube.com/embed/BdsCVvEYgHU?si=vvhaZJ8I5oXXvVJS?autoplay=1" title="Postiz Tutorial" allow="autoplay" allowFullScreen/>),
        });
    };
    const { data, isLoading } = useSWR(`/billing-${tier}-${period}`, loadCheckout, {
    ...
    const price = useMemo(() => Object.entries(pricing).filter(([key, value]) => key !== 'FREE'), []);
    const JoinOver = () => {
        return (<>
        ...
        {!!(user === null || user === void 0 ? void 0 : user.allowTrial) && (<div className="flex mt-[32px] mb-[10px] gap-[15px] tablet:mt-[32px] tablet:mb-[32px] text-[16px] font-[500] mobile:flex-col">
    ...
    return (<div className="blurMe flex flex-1 flex-col bg-newBgColorInner pb-[60px] mobile:pb-[100px]">
      <div className="h-[92px] px-[80px] tablet:px-[32px] mobile:!px-[16px] py-[20px] flex border-b border-newColColor">
        ...
        <div className="flex items-center">
          <div className="flex gap-[20px] text-textItemBlur">
            ...
            <div className="hover:text-newTextColor">
              {(user === null || user === void 0 ? void 0 : user.tier.current) === 'FREE' && (<LogoutComponent isIcon={true}/>)}
            </div>
          </div>
        </div>
      </div>
      <div className="flex px-[80px] tablet:px-[32px] mobile:!px-[16px] flex-1 flex-row tablet:flex-none tablet:flex-col-reverse">
        <div className="flex-1 py-[40px] tablet:pt-[80px] flex flex-col pe-[40px] tablet:pe-0">
          ...
          {!isLoading && data && stripe ? (<EmbeddedBilling stripe={stripe} secret={data.client_secret} showCoupon={period === 'MONTHLY'} autoApplyCoupon={data.auto_apply_coupon}/>) : (<LoadingComponent />)}
        </div>
          ...
          <div className="top-[20px] sticky">
            ...
            <div className="flex mb-[24px] mobile:flex-col">
              ...
              <div className="h-[44px] px-[6px] mobile:px-0 flex items-center justify-center mobile:justify-start gap-[12px] border border-newColColor rounded-[12px] select-none">
                <div className={clsx('h-[32px] mobile:flex-1 rounded-[6px] text-[16px] px-[12px] flex justify-center items-center', period === 'MONTHLY'
            ? 'bg-boxFocused text-textItemFocused'
            : 'cursor-pointer')} onClick={() => setPeriod('MONTHLY')}>
                  {t('billing_monthly', 'Monthly')}
                </div>
                <div className={clsx('gap-[10px] h-[32px] mobile:flex-1 rounded-[6px] text-[16px] px-[12px] flex justify-center items-center', period === 'YEARLY'
            ? 'bg-boxFocused text-textItemFocused'
            : 'cursor-pointer')} onClick={() => setPeriod('YEARLY')}>
            ...
            <div className="grid grid-cols-2 gap-[8px] mobile:!grid-cols-2 tablet:grid-cols-4">
              {price.map(([key, value]) => (<div onClick={() => setTier(key)} key={key} className={clsx('cursor-pointer select-none w-[266px] h-[138px] tablet:w-full tablet:h-[124px] p-[24px] tablet:p-[15px] rounded-[20px] flex flex-col', key === tier
...
export const BillingFeatures = ({ tier }) => {
    const t = useT();
    const features = useMemo(() => {
        ...
        list.push({
            ...
            prefix: currentPricing.posts_per_month > 10000
                ? 'unlimited'
        ...
        if (currentPricing === null || currentPricing === void 0 ? void 0 : currentPricing.ai) {
            list.push({
        ...
        if (currentPricing === null || currentPricing === void 0 ? void 0 : currentPricing.image_generator) {
            list.push({
                key: 'billing_ai_images_per_month',
                defaultValue: 'AI Images per month',
                prefix: currentPricing === null || currentPricing === void 0 ? void 0 : currentPricing.image_generation_count,
            });
        }
        if (currentPricing === null || currentPricing === void 0 ? void 0 : currentPricing.generate_videos) {
            list.push({
                key: 'billing_ai_videos_per_month',
                defaultValue: 'AI Videos per month',
                prefix: currentPricing === null || currentPricing === void 0 ? void 0 : currentPricing.generate_videos,
            });
        }
        return list;
    }, [tier]);
...
//# sourceMappingURL=first.billing.component.js.map