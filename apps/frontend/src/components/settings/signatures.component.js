...
export const SignaturesComponent = (props) => {
    ...
    const deleteSignature = useCallback((data) => () => __awaiter(void 0, void 0, void 0, function* () {
        if (yield deleteDialog(t('are_you_sure_you_want_to_delete', `Are you sure you want to delete?`, { name: data.content.slice(0, 15) + '...' }))) {
            yield fetch(`/signatures/${data.id}`, {
                method: 'DELETE',
            });
            mutate();
    ...
    return (<div className="flex flex-col">
      ...
      <div className="my-[16px] mt-[16px] bg-sixth border-fifth items-center border rounded-[4px] p-[24px] flex gap-[24px]">
        <div className="flex flex-col w-full">
          {!!(data === null || data === void 0 ? void 0 : data.length) && (<div className={`grid ${!!appendSignature
                ? 'grid-cols-[1fr,1fr,1fr,1fr,1fr]'
                : 'grid-cols-[1fr,1fr,1fr,1fr]'} w-full gap-y-[10px]`}>
              ...
              {data === null || data === void 0 ? void 0 : data.map((p) => (<Fragment key={p.id}>
                  ...
                  {!!appendSignature && (<div className="flex justify-center">
          ...
          <div>
            <Button onClick={addSignature()} className={clsx(((data === null || data === void 0 ? void 0 : data.length) || 0) > 0 && 'my-[16px]')}>
...
const AddOrRemoveSignature = (props) => {
    ...
    const form = useForm({
        ...
        values: {
            ...
            autoAdd: (data === null || data === void 0 ? void 0 : data.autoAdd) || false,
        },
    });
    ...
    const callBack = useCallback((values) => __awaiter(void 0, void 0, void 0, function* () {
        yield fetch((data === null || data === void 0 ? void 0 : data.id) ? `/signatures/${data.id}` : '/signatures', {
            method: (data === null || data === void 0 ? void 0 : data.id) ? 'PUT' : 'POST',
            body: JSON.stringify(values),
        });
        toast.show((data === null || data === void 0 ? void 0 : data.id)
            ? 'Signature updated successfully'
            : 'Signature added successfully', 'success');
        modal.closeCurrent();
        reload();
    }), [data, modal]);
    const t = useT();
    return (<FormProvider {...form}>
      <form onSubmit={form.handleSubmit(callBack)}>
        <div className="relative flex gap-[20px] flex-col flex-1 rounded-[4px] pt-0">
          <button className="outline-none absolute end-[20px] top-[15px] mantine-UnstyledButton-root mantine-ActionIcon-root hover:bg-tableBorder cursor-pointer mantine-Modal-close mantine-1dcetaa" type="button" onClick={() => modal.closeCurrent()}>
          ...
          <div className="relative bg-customColor2">
            <CopilotTextarea disableBranding={true} className={clsx('!min-h-40 !max-h-80 p-2 overflow-x-hidden scrollbar scrollbar-thumb-[#E8243C] bg-bigStrip outline-none')} value={text} onChange={(e) => {
            form.setValue('content', e.target.value);
        }} placeholder="Write your signature..." autosuggestionsConfig={{
            textareaPurpose: `Assist me in writing social media signature`,
            chatApiConfigs: {},
        }}/>
          </div>

          <Select label="Auto add signature?" translationKey="label_auto_add_signature" {...form.register('autoAdd', {
...
//# sourceMappingURL=signatures.component.js.map