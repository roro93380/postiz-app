...
const Icons = () => {
    return (<svg width="31" height="16" viewBox="0 0 31 16" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <defs>
        <linearGradient id="paint0_linear_2511_139661" x1="8" y1="0" x2="8" y2="16" gradientUnits="userSpaceOnUse">
          <stop stopColor="#18AFFF"/>
          <stop offset="1" stopColor="#0062DF"/>
        </linearGradient>
        <linearGradient id="paint1_linear_2511_139661" x1="23" y1="0" x2="23" y2="16" gradientUnits="userSpaceOnUse">
...
export const FacebookPreview = (props) => {
    ...
    const renderContent = topValue.map((p) => {
        const newContent = stripHtmlValidation('normal', p.content.replace(/<span.*?data-mention-id="([.\s\S]*?)"[.\s\S]*?>([.\s\S]*?)<\/span>/gi, (match, match1, match2) => {
            return `[[[${match2}]]]`;
        }), true);
        const { start, end } = textSlicer((integration === null || integration === void 0 ? void 0 : integration.identifier) || '', props.maximumCharacters || 10000, newContent);
        const finalValue = newContent
            .slice(start, end)
            .replace(/\[\[\[([.\s\S]*?)]]]/, (match, match1) => {
            return `<span class="font-bold font-[arial]" style="color: #00D4FF">${match1}</span>`;
        }) +
          ...
          <div className="flex items-center">
            ...
            <div>
              <svg width="20" height="20" viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                <path d="M12.5437 8L7.4563 8C7.05059 8 6.84741 8.56798 7.13429 8.90016L9.67799 11.8456C9.85583 12.0515 10.1442 12.0515 10.322 11.8456L12.8657 8.90016C13.1526 8.56798 12.9494 8 12.5437 8Z" fill="#A3A3A3"/>
              </svg>
            </div>
          </div>
          {renderContent.slice(1).map((value, index) => {
                var _a;
                return (<div key={index} className="flex flex-col gap-[12px]">
              <div className="flex gap-[6px] leading-[17px]">
                <div className="h-[34px]">
                  <img src={(integration === null || integration === void 0 ? void 0 : integration.picture) || '/no-picture.jpg'} alt="social" className="rounded-full relative z-[2] h-[34px] w-[34px]"/>
                </div>
                <div className="flex flex-col gap-[6px] min-w-[150px]">
                  <div className="flex flex-col gap-[4px] bg-bgCommentFacebook py-[8px] px-[12px] rounded-[12px]">
                    ...
                    <div className="whitespace-pre-line text-[14px] font-[400]" dangerouslySetInnerHTML={{
                        __html: value.text,
                    }}/>
                    {!!((_a = value.images) === null || _a === void 0 ? void 0 : _a.length) && (<div className="h-[100px] mt-[12px] -mx-[15px] overflow-hidden flex">
                        {value.images.map((image, index) => (<a key={`image_${index}`} className="flex-1" href={mediaDir.set(image.path)} target="_blank">
                            <VideoOrImage autoplay={true} src={mediaDir.set(image.path)}/>
                          </a>))}
                      </div>)}
                  </div>
                    ...
                    <div className="flex gap-[4px]">
                      ...
                      <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                          ...
                          <defs>
                            <linearGradient id="paint0_linear_2511_139920" x1="8" y1="0" x2="8" y2="16" gradientUnits="userSpaceOnUse">
                              <stop stopColor="#FF6680"/>
                              <stop offset="1" stopColor="#E61739"/>
                            </linearGradient>
                          </defs>
...
//# sourceMappingURL=facebook.preview.js.map